const fs = require("fs");

const data = [
  {
    id: 1,
    name: "Leanne Graham",
    username: "Bret",
    email: "Sincere@april.biz",
  },
  {
    id: 2,
    name: "Ervin Howell",
    username: "Antonette",
    email: "Shanna@melissa.tv",
  },
  {
    id: 3,
    name: "Clementine Bauch",
    username: "Samantha",
    email: "Nathan@yesenia.net",
  },
  {
    id: 4,
    name: "Patricia Lebsack",
    username: "Karianne",
    email: "Julianne.OConner@kory.org",
  },
  {
    id: 5,
    name: "Chelsey Dietrich",
    username: "Kamren",
    email: "Lucio_Hettinger@annie.ca",
  },
  {
    id: 6,
    name: "Mrs. Dennis Schulist",
    username: "Leopoldo_Corkery",
    email: "Karley_Dach@jasper.info",
  },
  {
    id: 7,
    name: "Kurtis Weissnat",
    username: "Elwyn.Skiles",
    email: "Telly.Hoeger@billy.biz",
  },
  {
    id: 8,
    name: "Nicholas Runolfsdottir V",
    username: "Maxime_Nienow",
    email: "Sherwood@rosamond.me",
  },
  {
    id: 9,
    name: "Glenna Reichert",
    username: "Delphine",
    email: "Chaim_McDermott@dana.io",
  },
  {
    id: 10,
    name: "Clementina DuBuque",
    username: "Moriah.Stanton",
    email: "Rey.Padberg@karina.biz",
  },
];
const setInitialData = () => {
  fs.writeFile("./people.txt", JSON.stringify(data), "utf-8", (err) => {
    if (err) console.log("Error saving data");
  });
};

const getData = () => {
  fs.readFile("./people.txt", "utf-8", (err, data) => {
    const parsed = JSON.parse(data);
    const names = parsed.map((row) => row.name);
    console.log(names);
  });
};

const getDetail = (id) => {
  fs.readFile("./people.txt", "utf-8", (err, data) => {
    const parsed = JSON.parse(data);
    const detail = parsed.find((row) => row.id === id);
    console.log(detail);
  });
};

module.exports = { setInitialData, getData, getDetail };

// ==========================================================================================

// const fs = require("fs");

// const writeData = () => {
//   const data = [
//     {
//       id: 1,
//       name: "Leanne Graham",
//       username: "Bret",
//       email: "Sincere@april.biz",
//     },
//     {
//       id: 2,
//       name: "Ervin Howell",
//       username: "Antonette",
//       email: "Shanna@melissa.tv",
//     },
//     {
//       id: 3,
//       name: "Clementine Bauch",
//       username: "Samantha",
//       email: "Nathan@yesenia.net",
//     },
//     {
//       id: 4,
//       name: "Patricia Lebsack",
//       username: "Karianne",
//       email: "Julianne.OConner@kory.org",
//     },
//     {
//       id: 5,
//       name: "Chelsey Dietrich",
//       username: "Kamren",
//       email: "Lucio_Hettinger@annie.ca",
//     },
//     {
//       id: 6,
//       name: "Mrs. Dennis Schulist",
//       username: "Leopoldo_Corkery",
//       email: "Karley_Dach@jasper.info",
//     },
//     {
//       id: 7,
//       name: "Kurtis Weissnat",
//       username: "Elwyn.Skiles",
//       email: "Telly.Hoeger@billy.biz",
//     },
//     {
//       id: 8,
//       name: "Nicholas Runolfsdottir V",
//       username: "Maxime_Nienow",
//       email: "Sherwood@rosamond.me",
//     },
//     {
//       id: 9,
//       name: "Glenna Reichert",
//       username: "Delphine",
//       email: "Chaim_McDermott@dana.io",
//     },
//     {
//       id: 10,
//       name: "Clementina DuBuque",
//       username: "Moriah.Stanton",
//       email: "Rey.Padberg@karina.biz",
//     },
//   ];
//   fs.writeFile("./result3.txt", JSON.stringify(data), "utf-8", (err) => {
//     if (err) console.log("Error Saving Data!");
//   });
// };

// const readData = () => {
//   fs.readFile("./result3.txt", "utf-8", (err, data) => {
//     const parsed = JSON.parse(data);
//     console.log(parsed);
//   });
// };

// module.exports = { writeData, readData };
